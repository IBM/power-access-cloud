...
def call(String buildResult) {
    ...
    // PR_HEAD_SHA is set right after checkout scm in each Jenkinsfile via:
    //   env.PR_HEAD_SHA = (checkout scm).GIT_COMMIT
    // This is the only reliable source — both GIT_COMMIT env var and CHANGE_SHA contain
    // the ephemeral pr-merge SHA Jenkins creates locally when using the pr-merge checkout
    // strategy, which does not exist on GitHub and causes a 422 on the statuses API.
